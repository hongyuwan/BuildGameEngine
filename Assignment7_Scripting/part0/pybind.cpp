...
//
// Mac:
// (May need to install python3.X on your system--I recommend with brew. May need to install XCode and XCode Commandline tools)
// clang++ -std=c++11 -shared -undefined dynamic_lookup -I./pybind11/include/ `python3.9 -m pybind11 --includes` pybind.cpp -o mymodule.so `python3.9-config --ldflags`
//
// Mysys/Windows:
